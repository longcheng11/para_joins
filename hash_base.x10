	...
	public static def main(args: Array[String]) {
		...
		finish for (p in Place.places()){
			at (p) async {
				...
				//counter for join
				//hit; no hit; number of edistribute for S; number of broadcast tuples; number of query keys; picked up skew keys
				join_counters(here.id)=new Array[Int](7);	
			}
		}
		...
		finish for( p in Place.places()){
			at (p) async {
				...
				for(s in S_list(here.id)){					
					des=hash_3(s.getK(),N);
					S_key_collector(des).add(s.getK());
					S_payload_collector(des).add(s.getP());
				}
		...
		finish for( p in Place.places()){
			at (p) {
				Console.OUT.println("Place: "+here.id+" "+join_counters(here.id)(0)+" "+join_counters(here.id)(1)+" "+join_counters(here.id)(2)+" "+join_counters(here.id)(3)+" "+join_counters(here.id)(4)+" "+join_counters(here.id)(5)+" "+join_counters(here.id)(6));
			}
		}		
	}
}
